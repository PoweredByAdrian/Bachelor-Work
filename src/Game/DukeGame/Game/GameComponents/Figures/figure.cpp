...
{
    this->cell = cell;
    this->setCoord(cell->getRow(), cell->getCol());
}

void Figure::setCoord(int x, int y){
    this->row = x;
    this->col = y;
}
...
bool Figure::isValidMove(GameState state, int row, int col)
{
    // Get all valid moves from markAvailableJumps
    MoveResult moves = this->markAvailableJumps(state);
