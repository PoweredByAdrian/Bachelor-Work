...
GameConfigure::GameConfigure()
    : playerA(nullptr), playerB(nullptr), bagPlayerA(nullptr), bagPlayerB(nullptr), dukeCellA(nullptr), dukeCellB(nullptr), guardPlayerA(false), guardPlayerB(false)
{
}
...
bool GameConfigure::canDrawFromBag(PlayerTeam player){
    ...
    if(!bag->isEmpty() && !this->getPlacableCellsForNewPiece(player).isEmpty()){
        return true;
    }
    return false;
}
